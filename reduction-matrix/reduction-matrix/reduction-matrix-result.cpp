...
int main(int argc, char * argv[]) {
	...
	if (nDim == TWO_DIMS) { // 2D matrix
		CudaMatrix<cudafloat> i2DMatrix;
		...
		for (int i = 0; i < nRows; i++)
			for (int j = 0; j < nColumns; j++) {
				if (!strcasecmp(argv[5], RANDOM_DATA)) {
					i2DMatrix(i, j) = (cudafloat)CUDA_VALUE(-1.0) + (CUDA_VALUE(2.0) * rand()) / RAND_MAX;
				}
				else {
					i2DMatrix(i, j) = (cudafloat)fCollectData++;
				}
			}
		...
		CudaMatrix<cudafloat> o2DMatrix;

		HostMatrix<cudafloat> h2DMatrix;
		...
		for (int i = 0; i < nRowsDim; i++)
			for (int j = 0; j < nColumnsDim; j++) {
				o2DMatrix(i, j) = (cudafloat)0;

				h2DMatrix(i, j) = (cudafloat)0;
			}
		...
		for (int i = 0; i < nRows; i++) {
			for (int j = 0; j < nColumns; j++) {
				if (nDimReduce == 1) {
					h2DMatrix(0, j) += (cudafloat)i2DMatrix(j, i);
				}
				if (nDimReduce == 2) {
					h2DMatrix(i, 0) += (cudafloat)i2DMatrix(j, i);
				}
			}
		}
	...
	if (nDim == THREE_DIMS) { // 3D matrix
		...
		for (int k = 0; k < nPlanes; k++)
			for (int i = 0; i < nRows; i++)
				for (int j = 0; j < nColumns; j++) {
					if (!strcasecmp(argv[5], RANDOM_DATA)) {
						i3DMatrix(i, j, k) = (cudafloat)CUDA_VALUE(-1.0) + (CUDA_VALUE(2.0) * rand()) / RAND_MAX;
					}
					else {
						i3DMatrix(i, j, k) = (cudafloat)fCollectData++;
					}
				}
		...
		CudaMatrix3D<cudafloat> o3DMatrix;

		HostMatrix3D<cudafloat> h3DMatrix;
		...
		o3DMatrix.ResizeWithoutPreservingData(nRowsDim, nColumnsDim, nPanelsDim);

		h3DMatrix.ResizeWithoutPreservingData(nRowsDim, nColumnsDim, nPanelsDim);

		// initialize CUDA and Host output matrices with zeroes
		for (int i = 0; i < nRowsDim; i++)
			for (int j = 0; j < nColumnsDim; j++) {
				for (int k = 0; k < nPanelsDim; k++) {
					o3DMatrix(i, j, k) = (cudafloat)0;

					h3DMatrix(i, j, k) = (cudafloat)0;
				}
			}
		...
		for (int i = 0; i < nRows; i++) {
			for (int j = 0; j < nColumns; j++) {
				for (int k = 0; k < nPlanes; k++) {
					if (nDimReduce == 1) {
						h3DMatrix(0, j, k) += (cudafloat)i3DMatrix(j, i, k);
					}
					if (nDimReduce == 2) {
						h3DMatrix(i, 0, k) += (cudafloat)i3DMatrix(j, i, k);
					}
					if (nDimReduce == 3) {
						h3DMatrix(i, j, 0) += (cudafloat)i3DMatrix(j, i, k);
					}
				}
			}
		}
		...
		if (bConsoleOut) { // print result to console
			...
			printf("\n%dx%dx%d reduce dim (%d) matrix:\n", nRowsDim, nColumnsDim, nPanelsDim, nDimReduce);
